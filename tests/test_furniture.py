...
from phototext.furniture import (signature, similar, printed_number, detect_furniture,
...
def page(lines, height=1600, status="done"):
    ...
    return SimpleNamespace(status=status, height=height, lines=out, regions=regions,
                           furniture=[], printed_page=None)


def test_signature_and_similarity():
    assert signature("6 | JOHN KOTTER AND HOLGER RATHGEBER") == "john kotter and holger rathgeber"
    assert similar(signature("10 | JOHN KOTTER AND HC"), signature("6 | JOHN KOTTER AND HOLGER RATHGEBER"))
...
def test_detects_alternating_footers_but_not_titles():
    ...
    p6 = page([("Our Iceberg Will Never Melt", 190, True)] + body + [("6 | JOHN KOTTER AND HOLGER RATHGEBER", 1540, False)])
...
def test_order_by_printed_page_fills_gaps():
    S = lambda k: SimpleNamespace(key=k, label=k)
    sources = [S("p8"), S("p6"), S("p7"), S("p10"), S("p9"), S("p11")]
