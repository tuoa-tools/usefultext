#!/usr/bin/env python3
"""PhotoText CLI — a folder of page photos (and PDFs) → text, markdown, JSONL, CSV.

    python doc_reader.py Documents/ --out output/

All processing is local and offline. The heavy lifting lives in the
`phototext` package; this file only parses arguments and prints progress.
"""
...
from phototext import Settings, discover_sources, run_job, sharpness_precheck, __version__
from phototext import ocr
from phototext.inputs import SORT_MODES
from phototext.preprocess import register_heif, heif_error, HEIF_EXTS


def build_parser() -> argparse.ArgumentParser:
    d = Settings()
    p = argparse.ArgumentParser(prog="doc_reader", description=__doc__,
    ...
    p.add_argument("--min-conf", type=float, default=d.min_page_conf,
                   help=f"page read-quality gate (default {d.min_page_conf})")
    p.add_argument("--region-conf", type=float, default=d.min_region_conf,
                   help=f"drop text regions below this read quality (default {d.min_region_conf})")
    p.add_argument("--blur-threshold", type=float, default=d.blur_threshold,
                   help=f"sharpness below this is flagged blurry (default {d.blur_threshold:g})")
    p.add_argument("--max-edge", type=int, default=d.max_long_edge,
    ...
    p.add_argument("--heading-ratio", type=float, default=d.heading_height_ratio,
    ...
    p.add_argument("--version", action="version", version=f"PhotoText {__version__}")
    return p


def settings_from_args(args) -> Settings:
    return Settings(pdf_dpi=args.dpi, sort=args.sort, max_long_edge=args.max_edge,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not sources:
        print("error: no supported files found (jpg/jpeg/png/heic/heif/tif/bmp/webp/pdf)", file=sys.stderr)
        return 2
    if any(s.path.suffix.lower() in HEIF_EXTS for s in sources) and not register_heif():
        print(f"warning: HEIC/HEIF files present but pillow-heif is not working: {heif_error()}",
              file=sys.stderr)

    title = args.title or (args.inputs[0].name if args.inputs[0].is_dir() else args.inputs[0].stem)

    say(f"PhotoText {__version__} — {len(sources)} page(s), ordered by {sort_used}")
    ...
    def on_page(rec, i, total, resumed):
        ...
        if rec.blurry:
            flags.append("blurry")
        say(f"[{i}/{total}] {rec.label}: {rec.n_regions} regions, read quality {rec.mean_conf:.2f}, "
            f"{rec.elapsed:.1f}s" + (f"  [{', '.join(flags)}]" if flags else ""))

    try:
        summary = run_job(sources, args.out, settings, on_page=on_page, force=args.force,
                          precheck=precheck, title=title)
    ...
    if low:
        ...
        for r in low:
            print(f"  page {r.page}: {r.label} (read quality {r.mean_conf:.2f}, {r.n_regions} regions)")
